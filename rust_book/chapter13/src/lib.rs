...
/// 
/// 
/// 尽管迭代器是一种高层次的抽象，但他在编译后生成了与手写底层代码几乎一样的产物。迭代器是
/// Rust语言中的一种零开销抽象, zero-cost abstraction, 这个词意味着我们在使用
/// 这些抽象时不会引入额外的运行时开销。
/// 
/// # 总结，
/// 
/// 闭包和迭代器是Rust受函数式编程语言启发而实现的功能，他们能帮助Rust在清晰
/// 地表达出高层次抽象概念的同时兼顾了底层性能，闭包哈迭代器的实现保证了运行时
/// 性能不会受到影响。这是Rust努力实现零开销这个目标的重要一环。
/// 
/// 
