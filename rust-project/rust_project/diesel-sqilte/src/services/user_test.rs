...
async fn get_user_from_api_by_id() {
    ...
    let create_resp: User = read_body_json(create_resp).await;  

    println!("/users/{}", create_resp.id);

    let resp = TestRequest::post()
        .uri(format!("/users/{}", create_resp.id).as_str())
        .send_request(&mut app)
        .await;
    ...
    assert_eq!(create_resp.id, retrieved_user.id);
}
...
async fn list_users_from_api() {
    ...
    let mut retrieved_user : Vec<User> =    serde_json::from_slice::<Vec<User>>(&body).unwrap();
    
    assert_eq!(retrieved_user.len(), 4);
    ...
    retrieved_user = serde_json::from_slice::<Vec<User>>(&body).unwrap();
