...
unsafe fn advance(bodies: &mut [body; BODIES_COUNT]){
    ...
    for i in 0..ROUNDED_INTERACTIONS_COUNT/2 {
        ...
        // let mut position_Delta = [__mm_setzero_pd(); 3];
        ...
        for m in 0..3 {
            ...
            // position_Delta[m] = *(&position_Delta[m].0 as *const f64 as *const __m128d).add(i);
        ...
        // acceptable results so two iterations of the Newton鈥揜aphson method are
...
fn output_Energy(bodies: &mut [body; BODIES_COUNT]){
    ...
    for i in 0..BODIES_COUNT {
        ...
        for j in i+1..BODIES_COUNT{
            ...
            for m in 0..3 {
                // position_Delta[m].as_mut_ptr().write(bodies[i].position[m] - bodies[j].position[m]);
